...
export const courseAttendanceStore = (set: StoreSet, get: StoreGet) => ({
  ...
  fetchAttendance: async (courseId: number, groupId: number) => {
    ...
    try {
      ...
      const attendanceWithGroupId = attendanceData.map(
  ...
  updateAttendance: async (
    ...
    try {
      ...
      const updatedAttendances = currentAttendances.map(
        (att: AttendanceResponse) =>
          att.id === updatedAttendanceWithGroupId.id
            ? updatedAttendanceWithGroupId
            : att
      )
