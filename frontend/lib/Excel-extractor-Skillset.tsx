...
const JP_TO_EN_MAP: Record<string, string> = {
  ...
  "その他（WindowsPhone、Tizen、Xamarin、Qt、Fluter）":
    "Other (Windows Phone, Tizen, Xamarin, Qt, Fluter)",
  "BIツール / Microsoft Power Automate/\nMicrosoft Power App / Tabular":
    "BI tools / Microsoft Power Automate / Microsoft Power App / Tabular",
  ...
  "Wicket ": "Wicket",
  "Ionic ": "Ionic",
  "Automation\n(RPA and Selenium web driver)":
    "Automation (RPA and Selenium web driver)",
  Shell: "shell",
  ...
  },
  {
    ...
    skill_sub_categories: [
      ...
      },
      {
        ...
        skills: [{ id: 58, skill_name: "Digital Ocean" }],
      },
      {
        id: 7,
...
for (const category of TECHNICAL_ABILITY_CONFIG) {
  for (const subCategory of category.skill_sub_categories) {
    for (const skill of subCategory.skills) {
      HEADERS_IN_ORDER.push(
        `technical ability - ${category.category_name} - ${subCategory.sub_category_name} - ${skill.skill_name} - Years`
      )
      HEADERS_IN_ORDER.push(
        `technical ability - ${category.category_name} - ${subCategory.sub_category_name} - ${skill.skill_name} - experience`
      )
    }
  }
}
  ...
  try {
    ...
    if (!targetWorksheet) {
      ...
      const availableSheets =
        sheetNames.length > 0 ? sheetNames.join(", ") : "No sheets found"
    ...
    if (targetWorksheet) {
      ...
      if (
        sheetName.includes("元データ") &&
        !sheetName.includes("Original data")
      ) {
    ...
    for (let r = dataStartRow; r <= totalRows; r++) {
      ...
      if (
        idVal.toLowerCase().includes("total") ||
      ...
      if (employees.length <= 10) {
        const techKeys = Object.keys(employeeData).filter(
          (key) => key.includes("Years") || key.includes("experience")
        )
      }
