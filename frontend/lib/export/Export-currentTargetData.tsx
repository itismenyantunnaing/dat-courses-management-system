  ...
  try {
    ...
    employeeJapaneseLevel_Data.forEach((profile: any, index: number) => {
      ...
      const rowData: { [key: string]: any } = {
        ...
        wantToSitExam:
          profile.wantToSitExam === true
            ? "Yes"
            : profile.wantToSitExam === false
              ? "No"
              : "-",
...
export const exportCurrentTargetToCSV = async (
  ...
  try {
    ...
    const defaultFileName =
      fileName || `CurrentTarget_${new Date().toISOString().split("T")[0]}.csv`
...
export const exportCurrentTargetToPDF = async (
  ...
  try {
    ...
    doc.text(
      "Existing Japanese Certified and Target Certified Level, Communication level",
      pageWidth / 2,
      15,
      { align: "center" }
    )
    ...
    doc.text(
      `Generated on: ${new Date().toLocaleString()} | Total Records: ${employeeJapaneseLevel_Data.length}`,
      14,
      22
    )
    ...
    // Data rows
    const rows = employeeJapaneseLevel_Data.map(
    ...
    const defaultFileName =
      fileName || `CurrentTarget_${new Date().toISOString().split("T")[0]}.pdf`
