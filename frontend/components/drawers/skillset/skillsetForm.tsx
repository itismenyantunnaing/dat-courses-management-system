...
export function SkillsetForm({
    ...
    const handleManagementChange = (field: keyof ManagementSkillData, value: string) => {
        onManagementSkillChange({
            ...managementSkillData,
    ...
    const handleTechnicalSkillChange = (
        ...
        if (existingIndex !== -1) {
            ...
            if (field === 'yearsOfExperience') {
                updated[existingIndex][field] = typeof value === 'string' ? parseFloat(value) || 0 : value
            ...
            for (const category of (skill_headers || [])) {
                for (const subCategory of (category.skillSubCategories || [])) {
                    const foundSkill = subCategory.skills?.find((s: any) => s.id === skillId || s.skillName === skillName)
                    if (foundSkill) {
            ...
            const newSkill: TechnicalSkillData = {
                ...
                yearsOfExperience: field === 'yearsOfExperience' ? (typeof value === 'string' ? parseFloat(value) || 0 : value) : 0,
            ...
            <div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                        <Label htmlFor="managementExperienceLevel">Management experience (Levels 1-5)</Label>
                        <Select
                            value={managementSkillData.managementExperienceLevel?.toString() || ""}
                            onValueChange={(value) => handleManagementChange('managementExperienceLevel', value)}
                        >
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="qcdScore">QCD (1-4 points)</Label>
                        <Select
                            value={managementSkillData.qcdScore?.toString() || ""}
                            onValueChange={(value) => handleManagementChange('qcdScore', value)}
                        >
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="reportConsultScore">Reporting, contacting, and consulting (1-4 points)</Label>
                        <Select
                            value={managementSkillData.reportConsultScore?.toString() || ""}
                            onValueChange={(value) => handleManagementChange('reportConsultScore', value)}
                        >
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="educationScore">Education (1-4 points)</Label>
                        <Select
                            value={managementSkillData.educationScore?.toString() || ""}
                            onValueChange={(value) => handleManagementChange('educationScore', value)}
                        >
                ...
                <div className="grid gap-6">
                    {developmentData.map((dev, index) => (
                        <div key={dev.developmentTypeName} className="space-y-3">
                            ...
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => handleInputChange(index, 'yearsOfExperience', e.target.value)}
                                        placeholder="e.g., 3.5"
                                        className="w-full"
                                    />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor={`process-${index}`}>Process Name</Label>
                                    <Input
                                        id={`process-${index}`}
                            ...
                            {index < developmentData.length - 1 && <Separator className="my-2" />}
                        </div>
                    ))}
                </div>
            </div>
                        ...
                        .map((category: any) => (
                            <div key={category.id} className="space-y-4">
                                ...
                                {(category.skillSubCategories || [])
                                    ...
                                    .map((subCategory: any) => (
                                        <div key={subCategory.id} className="pl-4 space-y-3">
                                            ...
                                            <div className="grid gap-4">
                                                {(subCategory.skills || [])
                                                    ...
                                                    .map((skill: any) => {
                                                        ...
                                                        return (
                                                            <div key={skill.id} className="grid gap-4 sm:grid-cols-3 bg-muted/30 p-4 rounded-md">
                                                                ...
                                                                <div className="space-y-2">
                                                                    ...
                                                                    <Input
                                                                        ...
                                                                        min="0"
                                                                        step="0.5"
                                                                        value={existingSkill?.yearsOfExperience ?? ""}
                                                                        onChange={(e) => {
