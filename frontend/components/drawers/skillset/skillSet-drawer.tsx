...
import { SkillsetForm, DevelopmentData, LanguageSkillData, ManagementSkillData, TechnicalSkillData } from "./skillsetForm"
...
export function SkillsetDrawer({
    ...
    const [originalDevelopmentData, setOriginalDevelopmentData] = useState<DevelopmentData[]>([])
    const [languageSkillData, setLanguageSkillData] = useState<LanguageSkillData>({
        languageSkillLevel: null
    })
    const [originalLanguageSkillData, setOriginalLanguageSkillData] = useState<LanguageSkillData>({
        languageSkillLevel: null
    })
    const [managementSkillData, setManagementSkillData] = useState<ManagementSkillData>({
        managementExperienceLevel: null,
        qcdScore: null,
        reportConsultScore: null,
        educationScore: null
    })
    const [originalManagementSkillData, setOriginalManagementSkillData] = useState<ManagementSkillData>({
        managementExperienceLevel: null,
        qcdScore: null,
        reportConsultScore: null,
        educationScore: null
    })
    const [technicalSkillData, setTechnicalSkillData] = useState<TechnicalSkillData[]>([])
    const [originalTechnicalSkillData, setOriginalTechnicalSkillData] = useState<TechnicalSkillData[]>([])
    ...
    useEffect(() => {
        if (open && employee) {
            ...
            const employeeDevCaps = devCap_data?.filter(
                (item: any) => item.employeeId === employee.id || item.employee_id === employee.id
            ) || []
            ...
            const initialData: DevelopmentData[] = types.map((type: any) => {
                ...
                return {
                    ...
                    yearsOfExperience: existing?.yearsOfExperience || existing?.years_of_experience || 0,
            ...
            const existingLanguageSkill = languageSkill_data?.find(
                (item: any) => item.employee_id === employee.id || item.employeeId === employee.id
            )

            const langData: LanguageSkillData = {
                languageSkillLevel: existingLanguageSkill?.language_skill_level || existingLanguageSkill?.languageSkillLevel || null
            }
            ...
            const mgmtData: ManagementSkillData = {
                managementExperienceLevel: existingManagementSkill?.management_experience_level || existingManagementSkill?.managementExperienceLevel || null,
                qcdScore: existingManagementSkill?.qcd_score || existingManagementSkill?.qcdScore || null,
                reportConsultScore: existingManagementSkill?.report_consult_score || existingManagementSkill?.reportConsultScore || null,
                educationScore: existingManagementSkill?.education_score || existingManagementSkill?.educationScore || null
            }
            ...
            const employeeTechnicalSkills = skillData?.filter(
                (item: any) => item.employee_id === employee.id || item.employeeId === employee.id
            ) || []

            const techData: TechnicalSkillData[] = employeeTechnicalSkills.map((skill: any) => ({
                ...
                subCategoryName: skill.sub_category_name || skill.subCategoryName || '',
                yearsOfExperience: skill.years_of_experience || skill.yearsOfExperience || null,
                experienceLevel: skill.experience_level || skill.experienceLevel || null,
            }))

            setTechnicalSkillData(techData)
            setOriginalTechnicalSkillData(JSON.parse(JSON.stringify(techData)))
        }
    }, [open, employee, devCap_headers, devCap_data, languageSkill_data, managementScores_Data, skillData])

    // Check if any data has changed
    const hasChanges = () => {
        const devChanged = JSON.stringify(developmentData) !== JSON.stringify(originalDevelopmentData)
        const langChanged = JSON.stringify(languageSkillData) !== JSON.stringify(originalLanguageSkillData)
        const mgmtChanged = JSON.stringify(managementSkillData) !== JSON.stringify(originalManagementSkillData)
        const techChanged = JSON.stringify(technicalSkillData) !== JSON.stringify(originalTechnicalSkillData)
    ...
    try {
        ...
        if (hasManagementData) {
            const mgmtData = {
                employeeId: employee.id,
                managementExperienceLevel: managementSkillData.managementExperienceLevel || 0,
        ...
        const existingTechnicalSkills = skillData?.filter(
            (item: any) => item.employee_id === employee.id || item.employeeId === employee.id
        ) || []
        ...
        for (const techSkill of technicalSkillData) {
            ...
            for (const category of (skill_headers || [])) {
                for (const subCategory of (category.skillSubCategories || [])) {
                    const foundSkill = subCategory.skills?.find((s: any) => s.skillName === techSkill.skillName)
                    if (foundSkill) {
            ...
            const existingTechSkill = existingTechnicalSkills.find(
                (item: any) => (item.skillName || item.skill_name) === techSkill.skillName
            )
            ...
            if (existingTechSkill) {
                ...
                console.log(`Updating technical skill ${techSkill.skillName}:`, techData)
                await update_SkillData(skillId, techData)
            } else {
                // Create new technical skill
                console.log(`Creating technical skill ${techSkill.skillName}:`, techData)
            ...
            <DrawerContent className="right-0 left-auto h-full w-full max-w-2xl">
                <DrawerHeader className="shrink-0 border-b">
                    ...
                    <div className="mt-2 flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        <span>Employee: {employee.name} ({employee.id})</span>
                    </div>
                </DrawerHeader>
