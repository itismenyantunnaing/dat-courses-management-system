...
export function ScheduleTab({ course, userRole }: ScheduleTabProps) {
  ...
  // Get user's enrolled groups for trainer courses
  const userEnrolledGroupIds = useMemo(() => {
    ...
    return groupIds
  }, [course, allEnrollments, currentUserId, isAdmin])

  // Derived sessions - filtered to only this course and user's enrolled groups
  const derivedSessions = useMemo<Session[]>(() => {
    ...
    const progressList = getProgressArray()
    ...
    return result
  }, [course, weekStart, allEnrollments, allStudyProgress, studyPeriodStart, isAdmin, userEnrolledGroupIds, currentUserId])
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        if (course.courseType === "trainer") {
          // Fetch enrollments and attendance for each group
    ...
    fetchData()
  }, [course.id, course.courseType, course.groups, fetch_courseEnrollments, fetchAttendance, fetch_studyProgress])

  // Initialize attendance and progress stores
  useEffect(() => {
    ...
    derivedSessions.forEach((s) => {
      if (s.type === "self-study") {
        ...
        initialProgress[s.id] = courseEnrollments.map((enrollmentRaw) => {
          ...
          const targetSession = findSelfStudyTargetSession([course], s.id)
  ...
  }, [derivedSessions, allEnrollments, allAttendances, allStudyProgress, course])
