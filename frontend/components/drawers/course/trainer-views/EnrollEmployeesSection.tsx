...
const isEnrollmentInTemporaryGroup = (enrollment: any, groups: any[]): boolean => {
...
export const EnrollEmployeesSection: React.FC<EnrollEmployeesSectionProps> = ({
    ...
    const { session, enrollments, fetch_courseEnrollments, enrollEmployee, unenrollEmployee } = mainStore()
    ...
    const redistributeEnrollmentsUI = useCallback((newGroups: any[]) => {
        ...
        try {
            ...
            if (currentGroupCount < prevGroupCount) {
                const remainingGroupIds = newGroups.map(g => parseInt(String(g.id).replace('g', '')));
                ...
                if (employeesFromRemovedGroups.length > 0 && remainingGroupIds.length > 0) {
                    ...
                    const sortedEmployees = [...employeesFromRemovedGroups].sort((a, b) => a.employeeId - b.employeeId);
                    let employeeIndex = 0;
                ...
                setDisplayEnrollments([...redistributedEnrollments, ...cancelledEnrollments]);
                setIsRedistributing(false);
                return;
            }
            ...
            const existingGroups = newGroups.filter(g => !isTemporaryGroupId(g.id));
            ...
            const existingGroupIds = existingGroups.map(g => parseInt(String(g.id).replace('g', '')));

            // Keep employees in existing groups
            const employeesInExistingGroups = activeEnrollments.filter(
            ...
            employeesNotInExistingGroups.forEach((enrollment: any, index: number) => {
            ...
            if (tempGroups.length > 0) {
                ...
                const employeesInExisting = activeEnrollments.filter(
                    (e: any) => existingGroupIds.includes(e.courseGroupId)
                );
                ...
                existingGroups.forEach((group: any, groupIndex: number) => {
                    ...
                    const targetCount = targetPerGroup + (groupIndex < remainder ? 1 : 0);

                    // If this group has more employees than target, move some to temp groups
                    if (employeesInThisGroup.length > targetCount && tempGroups.length > 0) {
                        ...
                        employeesToMove.forEach((enrollment: any, index: number) => {
                            const tempGroupIndex = index % tempGroups.length;
                            const tempGroup = tempGroups[tempGroupIndex];
                            const tempGroupId = parseInt(String(tempGroup.id).replace('g', ''));
    ...
    useEffect(() => {
        ...
        const prevGroupIds = prevGroupsRef.current.map(g => g.id).sort().join(',');
    ...
    const enrolledEmployees = useMemo(() => {
        ...
        if (isTrainer) {
            ...
            const groupId = parseInt(activeGroup.id.replace('g', ''))
            return displayEnrollments.filter((e: any) =>
    ...
    const handleAdminGroupChange = useCallback(async (enrollmentId: number, newGroupId: string) => {
        ...
        if (isEnrollmentInTemporaryGroup(employee, groups)) {
            alert('Cannot change group for an employee in a temporary group. Please save the course first.')
            setChangingEmployeeId(null);
            setSelectedNewGroupId("");
            setShowConfirmDialog(false);
            return
    ...
    }, [onAdminChangeGroup, courseId, fetch_courseEnrollments, onRefreshEnrollments, actualEnrollments, groups])
    ...
    const handleEnrollEmployee = useCallback(async (employee: MentionedLearner) => {
        ...
        if (isAlreadyEnrolled) {
            alert(`${employee.name} is already enrolled in this ${isTrainer ? 'group' : 'course'}`)
            return
        }

        if (isTrainer) {
            const existingEnrollment = actualEnrollments.find(
                (e: any) => e.employeeId === employee.id && e.enrollmentStatus !== 'CANCELLED'
            )
            if (existingEnrollment) {
                if (!confirm(`${employee.name} is currently in "${existingEnrollment.courseGroupName}". Do you want to move them to the current group?`)) {
                    return
                }
        ...
        if (!confirm(`Are you sure you want to enroll ${employee.name} in this ${isTrainer ? 'group' : 'course'}?`)) {
            return
        }
    ...
    }, [courseId, isTrainer, groups, activeGroupTab, currentGroupEnrolledIds, allEnrolledIds, actualEnrollments, enrollEmployee, unenrollEmployee, fetch_courseEnrollments, onRefreshEnrollments])
    ...
    const handleUnenrollEmployee = useCallback(async (enrollmentId: number, employeeName: string) => {
        ...
        if (!confirm(`Are you sure you want to unenroll ${employeeName} from this ${isTrainer ? 'group' : 'course'}?`)) {
            return
        }
    ...
    }, [courseId, isTrainer, unenrollEmployee, fetch_courseEnrollments, onRefreshEnrollments])
    ...
    if (isLoading || isRedistributing) {
        return (
            <div className="flex items-center justify-center py-8">
                ...
                {isRedistributing && (
                    <span className="ml-3 text-sm text-muted-foreground">Redistributing enrollments...</span>
                )}
            </div>
        )
    }
    ...
    return (
        <div className="space-y-4">
            ...
            {groupChangeSuccess && (
                <div className="rounded-lg border border-green-200 bg-green-50 p-2 text-sm text-green-600 flex items-center gap-2">
                    <span>✅</span>
                    {groupChangeSuccess}
                </div>
            )}
            ...
            {enrolledEmployees.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {enrolledEmployees.map((employee: any) => {
                        ...
                        const isInTemporaryGroup = isTrainer && isEnrollmentInTemporaryGroup(employee, groups);

                        const availableGroups = isTrainer ? groups.filter(
                            (g) => {
                                ...
                                const currentGroupId = typeof employee.courseGroupId === 'string'
                                            ...
                                            {isTrainer && (
                                                <>
                                                    ...
                                                    <span className="text-xs text-muted-foreground">
                                                        ...
                                                        {isInTemporaryGroup && (
                                                            <span className="ml-1 text-yellow-500">(Pending Save)</span>
                                                        )}
                                                    </span>
                                                </>
                                            )}
                                                    ...
                                                    <SelectContent position="popper" className="z-[9999]">
                                                        <SelectGroup>
                                                            {availableGroups.map((group) => {
                                                                const groupId = parseInt(String(group.id).replace('g', ''));
                                                                const groupEmployees = displayEnrollments.filter(
                                                                    (e: any) => e.courseGroupId === groupId && e.enrollmentStatus !== 'CANCELLED'
                                                                );
