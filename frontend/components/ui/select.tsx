...
function SelectContent({
  ...
  // Count valid SelectItem children (excluding disabled items)
  let itemCount = 0
  React.Children.forEach(children, (child) => {
    if (React.isValidElement(child)) {
      // Check if it's a SelectItem directly
      if (child.type === SelectItem && !child.props.disabled) {
        itemCount++
      }
      // Check inside SelectGroup
  ...
  // If forceOpen is true and there are no valid items, add a placeholder
  const shouldAddPlaceholder = forceOpen && itemCount === 0
